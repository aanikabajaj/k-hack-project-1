...
## Post-merge fixes — 2026-08-14 evening

Found after the combined merge landed, by re-reading the merged code against
what each side's source actually did (not by running it — same environment
limitation as above). Full write-up in
[changelog-2026-08-14.md](changelog-2026-08-14.md).

7. **Reschedule out of/into a full class mis-handled credits.** `reschedule`
   always copied the old booking's `creditsUsed` onto the new booking as-is.
   Fine for booked→booked, wrong on a status flip: booked→waitlisted kept
   the old charge (then charged *again* on promotion), waitlisted→booked
   inherited a $0 charge on a now-confirmed seat. **Fixed:** new booking's
   credits are now computed from the status it lands in - refunded on drop
   to waitlisted, charged (with a sufficiency check) on promotion to booked.

8. **Joining a waitlist required spare credits it never actually charged.**
   `bookings.book` checked `creditsRemaining < cls.creditCost` before
   checking whether the class was full, so a member with too few credits
   couldn't join a waitlist even though waitlisting charges nothing.
   **Fixed:** the credit check now only runs when the class isn't full.

9. **Schedule page let you re-attempt a booking you already had.** Classes
   you'd already booked or waitlisted still showed a clickable button,
   which the backend rejected with "You are already on the list for this
   class." **Fixed:** the page now shows a Booked/Waitlisted badge instead
   for classes you already have a status on, plus a success toast.

10. **Stale booking status could leak across a sign-out.** The fix for #9
    looks up your own status per class from `bookings.mine`, which is
    disabled while signed out - but React Query keeps serving its last
    cached result regardless of `enabled`. A tab that had been signed in
    and then signed out could keep showing the previous account's
    booked/waitlisted classes. **Fixed:** explicitly return an empty map
    when there's no signed-in user instead of trusting the query being
    disabled.

11. **NavBar showed member-only links to signed-out visitors and admins.**
    "Schedule" rendered unconditionally and "My bookings"/"Waitlist"
    rendered for any signed-in user, admins included. **Fixed:** all three
    now gated on `user && user.role !== "admin"`, matching the rest of the
    role-based nav.

12. **Reschedule modal rebuilt as a calendar picker.** The flat scrollable
    list of matching classes got hard to scan with more than a few dates.
    Replaced with a month-grid: dates with an available class are
    highlighted, picking one auto-selects the class or shows a time-picker
    if there's more than one that day. Added `formatTime()` to
    `lib/format.ts` for the time-picker.
